...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        self.test_mode = kwargs.pop("test", False)    # Flag to activate test mode

        if self.test_mode:
            self.package_mass = kwargs.pop("package_mass", 8)
            self.gravity = kwargs.pop("test_gravity", torch.tensor([0.0, -0.02]))
            self.fixed_package_position = kwargs.pop("test_package_position", 0.10)
            self.random_package_pos_on_line = False
            self.udr_enabled = False  # disattiva UDR in test
        else:
            self.package_mass = kwargs.pop("test_package_mass", 5)
            self.random_package_pos_on_line = kwargs.pop("random_package_pos_on_line", False)
            self.udr_enabled = kwargs.pop("udr_enabled", False)
            # add gravity parameter
            self.gravity = (0.0, -0.05)
            self.fixed_package_position = 0.0
        
        self.n_agents = kwargs.pop("n_agents", 3)   
        # Implementation UDR 
        self.udr_enabled = kwargs.pop("udr_enabled", False)  # Flag to activate UDR           
        self.min_package_mass = kwargs.pop("min_package_mass", 3.0)
        self.max_package_mass = kwargs.pop("max_package_mass", 7.0)
        self.min_gravity = kwargs.pop("min_gravity", 0.01)
        self.max_gravity = kwargs.pop("max_gravity", 0.1)


        if self.udr_enabled:
            self.random_package_pos_on_line = True
        ...
        # Make world
        world = World(batch_dim, device, gravity=self.gravity, y_semidim=1)
        # Add agents
    ...
    """Compute random values for package mass and gravity"""
    def _randomize_mass(self) -> float:
        return torch.rand(1).item() * (self.max_package_mass - self.min_package_mass) + self.min_package_mass

    def _randomize_gravity(self):
        return (0.0, -torch.rand(1).item() * (self.max_gravity - self.min_gravity) - self.min_gravity)
    

    def reset_world_at(self, env_index: int = None):

        if self.udr_enabled:
            self.package.mass = self._randomize_mass()
            self.world.gravity = self._randomize_gravity()
            ...
            [
                torch.zeros(
                    ...
                    (
                        ...
                        else self.fixed_package_position  #0.0
                    ),
                    (
                        self.line_length / 2 - self.package.shape.radius
                        if self.random_package_pos_on_line
