...
class Agent(object):
    ...
    def update_policy(self):
        ...
        if self.AC_critic == 'V':
            ...
            with torch.no_grad():
                target = rewards + self.gamma * self.critic(next_states).squeeze(-1) * (1 - done)

            critic_vals = self.critic(states).squeeze(-1)
            td_error = target - critic_vals
            ...
            # ---------------- Actor-Critic with Q(s,a) ----------------
            with torch.no_grad():
                next_actions = self.policy(next_states).mean
                target = rewards + self.gamma * self.critic(next_states, next_actions).squeeze(-1) * (1 - done)
            ...
            # Actor update: Policy Gradient with Q-value as Advantage estimate
            actor_loss = -(log_probs * td_error.detach()).mean()
        ...
        return actor_loss + critic_loss, td_error.detach().cpu().numpy()